...
function extractHeuristically(
  ...
  if (source === 'github') {
    ...
    const desc = gh.description || gh.packageJson?.description || `Ứng dụng ${cleanTitle} được phát triển bởi ${gh.owner || 'johnnyhoang'}.`;
...
- **Tác Giả (Author):** ${gh.owner || 'johnnyhoang'}
...
**${cleanTitle}** (${gh.fullName}) is developed by ${gh.owner || 'johnnyhoang'}, providing an integrated workspace solution.
...
`;
    ...
    return {
      ...
      category,
      database: hasSupabase ? 'JH Supabase Data 1' : 'JH Supabase NoData',
      status: 'Development',
      priority: 'Medium',
      author: gh.owner || 'johnnyhoang',
    ...
    return {
      ...
      priority: 'Medium',
      author: 'johnnyhoang',
      github: vc.gitRepo?.repo ? `https://github.com/${vc.gitRepo.org || 'johnnyhoang'}/${vc.gitRepo.repo}` : '',
...
export async function extractAppWithAI(
  ...
  const systemPrompt = `You are an expert Software Architect & Product Manager for johnnyhoang's software ecosystem.
Your mission is to analyze repository or deployment data and generate accurate, professional project metadata and technical specifications (SRS) for the App Store Workspace (JWallet).
...
3. Database: Select from ('JH Supabase Data 1', 'JH Supabase Data 2', 'JH Supabase NoData').
4. Status: Select from ('Production', 'Development', 'Staging', 'Planning').
5. Priority: Select from ('High', 'Medium', 'Low').
6. Author: GitHub owner / repository creator (default 'johnnyhoang').
      ...
      if (res.ok) {
        ...
        if (rawText) {
          ...
          return {
            ...
            database: parsed.database || 'JH Supabase Data 1',
            status: parsed.status || 'Development',
            priority: parsed.priority || 'Medium',
            author: parsed.author || (source === 'github' ? (data as GitHubRepoPayload).owner : 'johnnyhoang') || 'johnnyhoang',
